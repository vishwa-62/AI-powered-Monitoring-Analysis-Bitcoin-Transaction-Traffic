...
### Option 1: One-Command Container Deployment (Docker & Docker Compose)

The repository includes a complete production Docker Compose setup containerizing all 4 services:

```bash
# Clone the repository
git clone https://github.com/vishwa-62/AI-powered-Monitoring-Analysis-Bitcoin-Transaction-Traffic.git
cd AI-powered-Monitoring-Analysis-Bitcoin-Transaction-Traffic

# Build and start all 4 services (MySQL, FastAPI AI, Node Backend, Nginx Frontend)
docker compose up -d --build
```

#### Accessing Deployed Services:
- 🌐 **Frontend UI**: `http://localhost` (or `http://your-server-ip`)
- ⚙️ **Backend REST API**: `http://localhost:5000/api`
- 🤖 **AI Fast-API Service**: `http://localhost:8000/docs` (Interactive OpenAPI Swagger)
- 🗄️ **MySQL Database**: `localhost:3306` (Database: `bitcoin_monitoring`)
...
4. Deploy! (`client/vercel.json` will automatically route SPA pages and proxy `/api` calls).

#### 2. Deploy Backend & AI Service on Render:
1. Connect your repository to [Render.com](https://render.com).
2. Select **Blueprints** and load `render.yaml`.
3. Render will deploy the Python AI Microservice, Node.js API, and Static Frontend automatically.

#### 3. Deploy to AWS EC2 / DigitalOcean / Linode VPS:
```bash
